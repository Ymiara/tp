...
object ClimateService {
   ...
   * @return Boolean True
   */
  def isClimateRelated(description: String): Boolean = { description.toLowerCase.contains("global warming")||description.toLowerCase.contains("ipcc")||description.toLowerCase.contains("climate change")}
  ...
  def filterDecemberData(list: List[Option[CO2Record]]) : List[CO2Record] = {
    list
      .flatMap(recordOption => recordOption)
      .filter(record => record.month != 12)
  }
